...
class Resilience: 
    ...
    def motor(self, pos, e2c_flag, em_flag): 
        ## heli-modeに変える仕様は、selemodのActuatorクラスに反映させた方がいい
        ## 低レベルコードをこのメインプログラムでいじりたくない
        """
        Arguments
        -------------------------------------------------------------------
        pos(float) : counter value received from encoder
        ...
        if self.lower_lim <= pos < self.middle_lim1: 
            self.target_throttle = throttle1
        elif self.middle_lim1 <= pos < self.middle_lim2: 
            self.target_throttle = throttle2
        elif self.middle_lim2 <= pos < self.upper_lim: 
